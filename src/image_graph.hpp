...
public:
    ...
    double distance(const Point &p, Dir d) const {
        switch (d) {
        case Dir::RIGHT:
            return horiz.at<double>(p.y, p.x);
        case Dir::LEFT:
            return horiz.at<double>(p.y, p.x - 1);
        case Dir::TOPRIGHT:
            return pdiag.at<double>(p.y - 1, p.x);
        case Dir::BOTTOMLEFT:
            return pdiag.at<double>(p.y, p.x - 1);
        case Dir::TOP:
            return vert.at<double>(p.y - 1, p.x);
        case Dir::BOTTOM:
            return vert.at<double>(p.y, p.x);
        case Dir::TOPLEFT:
