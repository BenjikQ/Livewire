    ...
    , ui(new Ui::ImageViewer)
{
    ...
    statusBar()->showMessage(QString::number(size.width())
                             + " x " +
...
void ImageViewer::open()
{
    ...
    const QString filePath = QFileDialog::getOpenFileName(this, tr("Open Image"), homeDirectory, tr("Image Files (*.png *.jpg *.bmp)"));
...
void ImageViewer::save()
{
    static const QStringList homePath = QStandardPaths::standardLocations(QStandardPaths::HomeLocation);
    static const QString homeDirectory = homePath.first().split(QDir::separator()).last();
    const QString filePath = QFileDialog::getSaveFileName(this, tr("Open Image"), homeDirectory, tr("Image Files (*.png *.jpg *.bmp)"));
    if (!filePath.isEmpty())
        paintArea->save(filePath);
}
